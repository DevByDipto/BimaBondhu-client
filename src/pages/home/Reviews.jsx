      ...
      <div className="carousel w-full space-x-4 ">
        {slicedReviews.map((review, index) => (
          <div key={review._id} id={`slide${index}`} className="carousel-item relative w-full flex-shrink-0">
            <div className="card w-full bg-base-300 shadow-lg  border-blue-500 border-2">
              <div className="card-body items-center text-center">
                <img src={review.userPhoto} alt={review.userName} className="w-16 h-16 rounded-full border border-blue-500" />
            ...
            <div className="absolute flex justify-between transform -translate-y-1/2 left-4 right-4 top-1/2">
              <a href={`#slide${(index - 1 + slicedReviews.length) % slicedReviews.length}`} className="btn btn-circle bg-blue-500">❮</a>
              <a href={`#slide${(index + 1) % slicedReviews.length}`} className="btn btn-circle bg-blue-500">❯</a>
            </div>
          </div>
        ))}
      </div>
